...
def safe_literal_eval(val):
    if pd.isnull(val):
        return {}
    try:
        return ast.literal_eval(val) if val.strip().startswith("{") else {"raw": val}
...
def unnest_customers() -> Iterator[pd.DataFrame]:
    ...
    for df in source:
        ...
        df['address'] = df['address'].apply(safe_literal_eval)
        df['preferences'] = df['preferences'].apply(safe_literal_eval)
